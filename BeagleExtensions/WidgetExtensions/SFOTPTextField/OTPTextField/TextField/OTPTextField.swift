...
public final class OTPTextField: BaseInputView {
    ...
    let component: CustomOTPTextField?
    ...
    init(_ component: CustomOTPTextField, renderer: BeagleRenderer) {
        self.component = component
        self.controller = renderer.controller
        super.init(frame: .zero)
        updateUI()
        renderer.observe(component.otpString, andUpdate: \.text, in: self)
    }
...
private extension OTPTextField {
    ...
    func notifyIfTextChanged() {
        controller?.execute(actions: component?.onChange, with: "onChange", and: .string(text ?? ""), origin: self)
        onTextChanged?(text)
